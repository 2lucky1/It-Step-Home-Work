...
drawComplexTriangle(count);

function drawComplexTriangle(count) {
    ...
    for (let i = 0; i < partsNumber; i++) {
        if (i === 0) {
            drawTriangle(x1, y1, partsHeight);
            x1 -= indent;
            y1 += indent;
        }
        ...
        drawTrapeze(x1, y1, topBase, partsHeight);
...
function drawTriangle(x1, y1, height) {
    ctx.moveTo(x1, y1);
...
function drawTrapeze(x1, y1, topBase, height) {
    ctx.moveTo(x1, y1);
